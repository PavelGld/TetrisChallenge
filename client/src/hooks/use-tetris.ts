...
export function useTetris() {
  ...
  const initializeGame = useCallback(() => {
    ...
    setCurrentPosition({ 
      x: Math.floor((10 - firstPiece.shape[0].length) / 2), 
      y: 0 
    });
  ...
  }, [gameBoard, currentPiece, currentPosition, isGameActive, isPaused]);
  ...
  const handlePieceLanded = useCallback(() => {
    ...
    if (newPiece && checkCollision(
      clearedBoard, 
      newPiece.shape, 
      { x: Math.floor((10 - newPiece.shape[0].length) / 2), y: 0 }
    )) {
      handleGameOver();
      return;
    }
    ...
    setCurrentPosition({
      x: Math.floor((10 - (newPiece?.shape[0].length || 0)) / 2),
      y: 0
    });
